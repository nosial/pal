<?php
    ...
    class Autoloader
    {
        ...
        private static function buildAutoloaderSource(array $mapping, array $options, string $directoryPath): string
        {
            ...
            $preDefinition = $options['pre_definition'] ?? '';
            $postDefinition = $options['post_definition'] ?? '';
            ...
            // Process pre_definition and post_definition code
            $preDefinitionCode = '';
            if (!empty($preDefinition))
            {
                $sanitizedPre = self::sanitizePhpCode($preDefinition);
                if (!empty($sanitizedPre))
                {
                    $indentedPre = self::indentCode($sanitizedPre, 1);
                    $preDefinitionCode = "\n    // Custom pre-definition code\n" . $indentedPre . "\n";
                }
            }
            ...
            {
                $sanitizedPost = self::sanitizePhpCode($postDefinition);
                if (!empty($sanitizedPost))
                {
                    $indentedPost = self::indentCode($sanitizedPost, 1);
                    $postDefinitionCode = "\n    // Custom post-definition code\n" . $indentedPost . "\n";
                }
            }
            ...
            $source = "<?php\n";
            $source .= "/**\n";
            $source .= " * Generated Standalone Autoloader\n";
            $source .= " * \n";
            $source .= " * This autoloader was generated by PAL Autoloader on {$timestamp}\n";
            $source .= " * Total classes: {$classCount}\n";
            $source .= " * Static files: {$staticFileCount}\n";
            $source .= " * Case insensitive: {$caseInsensitiveText}\n";
            $source .= " * Prepend: {$prependText}\n";
            $source .= " * Relative paths: {$relativeText}\n";
            $source .= " * Include static: {$includeStaticText}\n";
            $source .= " * \n";
            $source .= " * Usage: Simply require or include this file to register the autoloader\n";
            $source .= " * \n";
            $source .= " * @see https://git.n64.cc/nosial/pal\n";
            $source .= " * @see https://github.com/nosial/pal\n";
            $source .= " * @see https://codeberg.org/nosial/pal\n";
            $source .= " * @generated\n";
            $source .= " */\n\n";
            $source .= "// Prevent multiple registrations of the same autoloader\n";
            $source .= "if (!defined('{$autoloaderId}')) \n";
            $source .= "{\n";
            $source .= $preDefinitionCode;
            $source .= "    define('{$autoloaderId}', true);\n";
            $source .= "    \n";
            $source .= "    // Class to file mapping\n";
            $source .= "    \$GLOBALS['{$autoloaderId}_mapping'] = {$mappingCode};\n";
            $source .= "    \n";
            $source .= "    // Static files to include\n";
            $source .= "    \$GLOBALS['{$autoloaderId}_static_files'] = {$staticFilesCode};\n";
            $source .= "    \n";
            $source .= "    // Configuration\n";
            $source .= "    \$GLOBALS['{$autoloaderId}_case_insensitive'] = {$caseInsensitiveText};\n";
            $source .= "    \$GLOBALS['{$autoloaderId}_include_static'] = {$includeStaticText};\n";
            $source .= "    \n";
            $source .= "    // Include static files immediately if enabled\n";
            $source .= "    if (\$GLOBALS['{$autoloaderId}_include_static'])\n";
            $source .= "    {\n";
            $source .= "        foreach (\$GLOBALS['{$autoloaderId}_static_files'] as \$staticFile)\n";
            $source .= "        {\n";
            $source .= "            if (is_file(\$staticFile) && is_readable(\$staticFile))\n";
            $source .= "            {\n";
            $source .= "                require_once \$staticFile;\n";
            $source .= "            }\n";
            $source .= "        }\n";
            $source .= "    }\n";
            $source .= "    \n";
            $source .= "    // Autoloader function\n";
            $source .= "    \$GLOBALS['{$autoloaderId}_loader'] = function(\$className) \n";
            $source .= "    {\n";
            $source .= "        \$mapping = \$GLOBALS['{$autoloaderId}_mapping'];\n";
            $source .= "        \$caseInsensitive = \$GLOBALS['{$autoloaderId}_case_insensitive'];\n";
            $source .= "        \$filePath = null;\n";
            $source .= "        \n";
            $source .= "        if (\$caseInsensitive)\n";
            $source .= "        {\n";
            $source .= "            foreach (\$mapping as \$mappedClass => \$mappedFile)\n";
            $source .= "            {\n";
            $source .= "                if (strcasecmp(\$mappedClass, \$className) === 0)\n";
            $source .= "                {\n";
            $source .= "                    \$filePath = \$mappedFile;\n";
            $source .= "                    break;\n";
            $source .= "                }\n";
            $source .= "            }\n";
            $source .= "        }\n";
            $source .= "         else \n";
            $source .= "        {\n";
            $source .= "            if (isset(\$mapping[\$className]))\n";
            $source .= "            {\n";
            $source .= "                \$filePath = \$mapping[\$className];\n";
            $source .= "            }\n";
            $source .= "        }\n";
            $source .= "        \n";
            $source .= "        if (!\$filePath || !is_file(\$filePath) || !is_readable(\$filePath)) \n";
            $source .= "        {\n";
            $source .= "            return false;\n";
            $source .= "        }\n";
            $source .= "        \n";
            $source .= "        try \n";
            $source .= "        {\n";
            $source .= "            require_once \$filePath;\n";
            $source .= "            return true;\n";
            $source .= "        } \n";
            $source .= "        catch (\\Exception \$e)\n";
            $source .= "        {\n";
            $source .= "            trigger_error(\"Autoloader: Failed to load '{\$filePath}': \" . \$e->getMessage(), E_USER_WARNING);\n";
            $source .= "            return false;\n";
            $source .= "        }\n";
            $source .= "    };\n";
            $source .= "    \n";
            $source .= "    // Register the autoloader\n";
            $source .= "    spl_autoload_register(\$GLOBALS['{$autoloaderId}_loader'], true, {$prependText});\n";
            $source .= $postDefinitionCode;
            $source .= "}\n";
        ...
        /**
         * Sanitizes PHP code by removing PHP tags
         * 
         * Removes opening PHP tags
         * from the given code to prevent syntax errors when embedding code.
         * Handles edge cases where tags may appear mid-code or at start/end.
         * 
         * @param string $code The PHP code to sanitize
         * @return string The sanitized code without PHP tags
         */
        private static function sanitizePhpCode(string $code): string
        {
            if (empty($code))
            {
                return '';
            ...
            // Remove opening PHP tags (<?php, <?PHP, <?)
            // We need to be careful about the order and positioning
            $code = preg_replace('/^\s*<\?(?:php|PHP)?\s*/s', '', $code);
            $code = preg_replace('/<\?(?:php|PHP)?\s*/s', '', $code);
            
            // Remove closing PHP tag () - be careful as it could be in strings
            // We'll remove it only if it's at the end or followed by whitespace
        ...
        /**
         * Indents code blocks with proper spacing
         * 
         * Takes multi-line code and adds the specified number of indentation 
         * levels (4 spaces per level) to each line. Preserves existing relative
         ...
         */
        private static function indentCode(string $code, int $levels = 1): string
        {
            if (empty($code))
            {
                return '';
